...
test("start accepts an explicit example input", async () => {
  const run = { runId: "run-example", status: "passed" };
  const submission = { status: "pending_human" };
  const { state } = createState({ input: "旧输入" });

  await withFetchMock((path) => {
    if (path === "/api/runs") return jsonResponse(200, run);
    if (path === "/api/runs/run-example/clarification-history") {
      return jsonResponse(200, { runId: "run-example", entries: [] });
    }
    if (path === "/api/runs/run-example/submission") return jsonResponse(200, submission);
    throw new Error(`Unexpected request: ${path}`);
  }, async (requests) => {
    await createConsoleActions(state).start("给文章列表加阅读量展示");
...
test("start falls back to state input when called as a click handler", async () => {
  const run = { runId: "run-click", status: "passed" };
  const submission = { status: "pending_human" };
  const { state } = createState({ input: "给文章列表加阅读量展示" });
  const clickEvent = { currentTarget: { tagName: "BUTTON" }, type: "click" };

  await withFetchMock((path) => {
    if (path === "/api/runs") return jsonResponse(200, run);
    if (path === "/api/runs/run-click/clarification-history") {
      return jsonResponse(200, { runId: "run-click", entries: [] });
    }
    if (path === "/api/runs/run-click/submission") return jsonResponse(200, submission);
    throw new Error(`Unexpected request: ${path}`);
  }, async (requests) => {
    await createConsoleActions(state).start(clickEvent);
