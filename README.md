...
本仓库只保存“实现一个可以端到端交付全栈项目的超级个体”课题的 `v1.0.0` 设计基线。

**Agent 可以进行代码相关工作**（实现、调试、测试、运行验证、生成 run 证据等），但**必须在实现仓库** [`bytedance-implementation/`](./bytedance-implementation/)（绝对路径：`/Users/xioshark/Desktop/bytedance/bytedance-implementation`）内进行，不得在本仓库根目录新建或修改 `apps/`、`services/`、`sandbox-repo/` 等业务源码。
...
本仓库是**设计基线文档仓**，不含可运行业务代码。所有课题说明集中在 [`docs/`](./docs/) 一个目录里，按端到端交付流程编号 `01`–`12`；**改文档只改 `docs/` 里对应文件**，不要在根目录另建副本。
...
| 2 | [`AGENTS.md`](./AGENTS.md) | Agent / 协作者的工作边界：能改哪些路径、代码必须去哪个实现仓 |
| 3 | [`docs/`](./docs/) 按 **01 → 12** 顺序 | 课题的完整设计基线（下文逐份说明） |

源题原文（只读、不转写密钥）：[`实现一个可以端到端交付全栈项目的“超级个体”.pdf`](./实现一个可以端到端交付全栈项目的“超级个体”.pdf)。派生理解以 [`docs/01-understanding.md`](./docs/01-understanding.md) 为准。
...
| Agent 首次进场 | **AGENTS.md** → 01 → 03 → 05 → 06 |
...
- **运行证据与提交草稿**：在实现仓 `docs/reports/runs/` 与 `docs/reports/submission/`，不在本仓库根目录。
...
| 位置 | Agent 可做什么 | 不可做什么 |
|------|----------------|------------|
| 本仓库（`bytedance/`） | 阅读与维护设计基线：`README.md`、`AGENTS.md`、`docs/**` | 编写或修改业务源码、Conduit sandbox、run 证据、Demo |
| 实现仓库（[`bytedance-implementation/`](./bytedance-implementation/)） | **全部代码工作**：实现、重构、单测、lint、`npm run verify` / `run:p0`、运行报告与提交材料 | 把真实 API key 写入仓库；在本仓库外另建平行实现目录 |

实现仓库路径：`/Users/xioshark/Desktop/bytedance/bytedance-implementation`。进行任何代码相关操作前，应将工作目录切换到该路径（或等价地使用该路径下的文件绝对路径）。
...
2. **在 `bytedance-implementation/` 下进行所有代码与验证工作**；若目录不存在则在该路径创建，不要在本仓库根目录落业务源码。
...
- [ ] **§2.2 #1 抽象到位**：≥2 Skill；第 3 模式只加 1 个 Skill 文件
- [ ] **§2.2 #2 断点重放**：`resume-from-stage` + 只重放下游演示
- [ ] **§2.2 #3 跨栈一致性**：L2 跨栈 run + 影响矩阵
- [ ] **§2.2 #4 可观测性**：LLM `ai-calls.jsonl` + 监控面板
- [ ] **§2.2 #5 业务上下文反哺**：相似 run 召回入 plan
- [ ] **§2.2 #6 澄清深度**：模糊输入 + 真实 LLM 追问
- [ ] **§8.2**：公开 AI 主仓 + `sandbox-repo/`、Demo、视频、团队信息

## 源题 PDF §7.1 忽略

阅读 [`实现一个可以端到端交付全栈项目的“超级个体”.pdf`](./实现一个可以端到端交付全栈项目的“超级个体”.pdf) 时，**整节忽略 §7.1（官方资源池）**：含 TRAE / 火山方舟配置说明、示例 EP / API key、curl 示例、调用配额与封禁规则、IDE 推荐等。上述内容不构成本仓库或实现仓库的设计、验收或实现义务。
...
- **提交 / 答辩**：clarify **至少一次真实 LLM**（豆包或其它 API），在实现仓 `ai-usage.md` 声明；**不强制豆包**。
...
| 问题 | 唯一事实源 |
|------|------------|
| 源题是什么 | [`实现一个可以端到端交付全栈项目的“超级个体”.pdf`](./实现一个可以端到端交付全栈项目的“超级个体”.pdf) |
...
| 目录 | 内容 |
|------|------|
| 本仓库（`bytedance/`） | 设计基线、[`docs/`](./docs/)（`01`–`12` 流程文档）、源题 PDF；**不含**业务源码与 run 证据 |
| [`bytedance-implementation/`](./bytedance-implementation/) | 前端、后端、编排、Skill、sandbox-repo、`docs/reports/`、提交材料；**Agent 代码工作的唯一目录** |

除源题 PDF 原文外，本仓库根目录不包含业务源码、Conduit fork、运行报告、PR 证据、Demo、视频、**临时 CSV 执行台账**或 Agent Spec 目录。冲刺任务只维护 [`docs/06-plan.md#冲刺关键路径`](./docs/06-plan.md#冲刺关键路径进度-ssot) 与 [`docs/10-progress.md`](./docs/10-progress.md)。
