...
| M6 | 第 18-21 天 | §2.2 能力闭合 | 第 2/3/4 Skill、L2 跨栈 run、Skill 扩展演示 |
...
- [x] 执行 Conduit 仓库真实单测。
- [x] Skill `validation` 显式声明 `npm run lint:sandbox` 后，执行针对本次 Conduit 改动的 adapter，并在验证结果中标记来源；未声明时按 gap/fail 处理。
- [x] 生成验证报告。
- [x] 生成 PR 标题和描述。
- [x] 实现 GitHub draft PR provider；真实远端 PR 仍依赖人工提供 GitHub 配置、fork/分支权限和显式确认，属于 H17 可选项。
...
- [x] 监控面板汇总 tokens/延迟/成本（§2.2 #4，H4 验收 run 已归档；H13 面板已就绪）；S7 仍须录屏展示该证据。
...
- [ ] 公开 AI 系统主仓（`bytedance-implementation/`），且仓内包含 `sandbox-repo/`；团队信息、分工、亮点（映射 §2.2）。

## §2.2 六项追踪

| # | 亮点 | 代码 / run 证据 | S7 视频 | 证据边界 |
|---|------|----------------|---------|----------|
| 1 | 抽象到位 | ✅ 4 Skill；第 3/4 个仅新增 Skill 文件 | 🟡 待录屏 | 演示只加 Skill 文件不改 Orchestrator/Agent 主干 |
| 2 | 断点重放 | ✅ `resume-from-stage` + checkpoints + Web 入口 | 🟡 待录屏 | 录制 plan 后只重跑 edit → verify → pr |
| 3 | 跨栈一致性 | ✅ L2 run `run-2026-05-21T05-52-12-490Z` + 影响矩阵 | 🟡 待录屏 | 固定 L2 exemplar + `article-draft-indicator` 一致性检查器；不是通用自动跨栈同步框架 |
| 4 | 可观测性 | ✅ LLM ai-calls 非零 metrics + 面板汇总 | 🟡 待录屏 | **不可降级**；rules tokens=0 不计入该项 |
| 5 | 业务上下文反哺 | ✅ 相似 run plan 引用召回 | 🟡 待录屏 | UI 展示召回 + plan `history_references` |
| 6 | 澄清深度 | ✅ 模糊输入 LLM 追问 run | 🟡 待录屏 | **不可降级**；清晰 L1 rules run 不计入该项 |
...
H1–H16 的代码与 run 证据已闭合；当前不再按 Day 1–16 开发计划推进。剩余顺序只保留提交线：S7 录制 §2.2 六项视频 → S8 发布公开 AI 系统主仓 → S6 填写团队 / Demo / 视频 / 仓库 URL → S10 提交 §8.2 材料。

## 冲刺关键路径（进度 SSOT）

**唯一执行清单**：本表 + [`10-progress.md`](./10-progress.md)（事实快照）。根目录不保留临时 CSV；[`当前进度汇报.md`](../当前进度汇报.md) 仅为可读摘要，变更须先写本表与 `10-progress`。

**口径**：代码级 P0 ≠ 课题完成。早期 `aiMode: doubao` 探索 run（如 `run-2026-05-20T17-37-55-856Z`）有非零 tokens，但输入为清晰 L1 原句，**不计入 §2.2 #6**；课题完成须 **模糊输入 + LLM 主动追问** 的验收 run。**不强制豆包**（§7.2 多模型声明）；团队决策 P1-5 跳过，验收 run 为 `run-2026-05-21T05-58-01-181Z`（`mimo-v2.5`）。

| id | 优先级 | 任务 | 状态 | 依赖 | 目标窗口 | 映射 |
|----|--------|------|------|------|----------|------|
| H1 | **P0** | 隔离缺 `requirement.md` / `plan.md` 的坏归档影响 | **已完成** | — | Day 1 | `history-recall` 明示 `degraded` / `skipped`，不伪装全量召回；旧坏归档仍可作为失败证据保留 |
...
| H14 | **P0** | §2.2 新能力落地后 `npm run verify` 全绿 | **已完成** | H4–H12 | 持续 | 108 项 Node/API/Web/scripts 测试（107 pass / 1 skip）+ sandbox lint + Vitest + web build |
| H15 | **P0** | 对照 [`11-acceptance.yaml`](./11-acceptance.yaml) 逐项勾选 | **已完成** | H14 | Day 15–17 | 24/28 AC verified；AC-F006-02/AC-F011-01 partial，AC-F011-02/03 manual_pending；F-011/S7/S8 人工待闭合 |
| H16 | **P0** | 对照 [`12-traceability.yaml`](./12-traceability.yaml) 更新追踪 | **已完成** | H15 | Day 15–17 | F-001/F-008/F-009/F-012 已同步 evidence |
| H17 | **P2 可选** | 配置 GitHub 凭据、fork/分支权限并创建真实 draft PR（归档 URL） | **暂缓** | — | Day 14–16 | 上游 Conduit PR 非 §8.2 必填；本地 pr-draft 即可 |
...
| S6 | 提交 | 人工填写 submission：团队/分工/Demo/视频/仓库链接 | **部分** | H12,H15 | Day 17–19 | `team-info.md` 已生成；URL/姓名待填 |
| S7 | 提交 | 录制 3–8 分钟演示视频（覆盖 §2.2 六项 + §7.2 留痕） | **部分** | H9,H7,H11,H12,H4 | Day 18–20 | `video-recording-guide.md` 已生成；视频待录 |
| S8 | 提交 | 公开 AI 系统主仓，且仓内包含 `sandbox-repo/` Conduit 代码目录 | **部分** | H14 | Day 17–20 | `public-repo-guide.md` 已生成；远端待发布 |
| S9 | 提交 | 提交前最终安全检查：无真实 API key/EP 入库 | **部分** | S8 | 6.10 前 | `security-check-report.md` 已生成；公开发布当天人工再检待完成，仍须重跑 `pre-submission-check.sh`、secret scanning 与远端检查 |
| S10 | 提交 | 6.10 前提交 §8.2 材料并准备答辩 | **部分** | S6–S9 | **2026-06-10** | `defense-prep.md`；对外链接待闭合 |
| X1 | 风险 | `sandbox-repo` 依赖 audit 决策（fix 与否）并全量 verify | **已完成** | — | 提交前 | `dependency-audit-decision.md`；verify 已复跑 |
...
| 风险 | 影响 | 处理 |
|------|------|------|
| Conduit 原仓 lint 脚本缺失 | P0 lint 口径含混 | Skill `validation` 显式声明 `lint:sandbox` 才使用 adapter；未声明直接 gap/fail |
...
**§2.2 代码 / run 证据（已闭合，答辩仍须录屏展示）：**

- [x] §2.2 #1：4 Skill；第 3/4 模式只加 Skill 文件，不改 Orchestrator/Agent 主干。
- [x] §2.2 #2：`resume-from-stage` + checkpoints + Web 入口。
- [x] §2.2 #3：L2 跨栈 run `run-2026-05-21T05-52-12-490Z`。
- [x] §2.2 #4：LLM 非零 metrics + Web AI Usage 面板。
- [x] §2.2 #5：相似 run 召回写入 plan。
- [x] §2.2 #6：模糊输入 LLM 澄清 run `run-2026-05-21T05-58-01-181Z`。

**课题完成（仍待人工 / 外部 gate）：**

- [ ] S7：3–8 分钟演示视频覆盖 §2.1 主链路和 §2.2 六项。
- [ ] S8：公开 AI 系统主仓（内含 `sandbox-repo/`）。
- [ ] S6：团队信息、Demo URL、视频 URL、公开仓 URL 填入 `team-info.md`。
- [ ] S10：6.10 前提交 §8.2 材料。
