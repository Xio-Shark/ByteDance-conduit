...
|----------|------|----------|----------|------|
| `deepseek-v4-flash` | **当前默认主路径**：clarify + plan 阶段真实模型调用（OpenAI 兼容接口 `https://api.deepseek.com`；可经 `LLM_*` 替换为豆包 EP 或其他兼容服务）。**最新验收 run**：`run-2026-06-10T07-45-17-125Z`，clarify 423/589 tokens（6.8s），plan 1083/1366 tokens（20.3s） | 个人 API 账户 | 否 | **已用（提交截止日 2026-06-10 最终验收）** |
