...
import { checkCrossStackSync, loadChangedFileContents } from "../../checks/src/crossStackSync.js";
...
export async function verifyRun({ sandbox, skill, changedFiles = [] }) {
  ...
  if (skill?.id === "article-draft-indicator" && changedFiles.length) {
...
async function runCrossStackCheck({ sandbox, skill, changedFiles }) {
  try {
    ...
    const result = checkCrossStackSync({ skill, sandboxFiles });
...
function crossStackResult(result) {
  return {
    ...
    durationMs: 0,
    summary: result.message || "Cross-stack draft field consistency check",
  };
}
